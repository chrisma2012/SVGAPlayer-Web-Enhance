...
export class Renderer {
            ...
            var sound = new Howl({
              src: [
                navigator.vendor === "Google Inc."
                  ? URL.createObjectURL(this.dataURLtoBlob("data:audio/x-mpeg;base64," + src))
                  : "data:audio/x-mpeg;base64," + src,
              ],
  ...
  clear() {
    const targetCanvas = this._owner._drawingCanvas || this._owner._container;
    // 由于Player的destroy方法会删除_drawingCanvas属性，因为targetCanvas的值可能为HTMLDIVElement的实例，导致getContext出错。
  ...
  drawFrame(frame) {
    if (this._prepared) {
      ...
      sprites.forEach((sprite, index) => {
        ...
        if (
          isMatteing &&
          (sprite.matteKey == null || sprite.matteKey.length == 0 || sprite.matteKey != lastSprite.matteKey)
        ) {
          isMatteing = false;
        ...
        if (
          ...
          (lastSprite.matteKey == null || lastSprite.matteKey.length == 0 || lastSprite.matteKey != sprite.matteKey)
        ) {
          isMatteing = true;
          ctx.save();
        }
  ...
  drawSprite(sprite, ctx, frameIndex) {
    ...
    let src =
      this._owner._dynamicImage[bitmapKey] || this._bitmapCache[bitmapKey] || this._owner._videoItem.images[bitmapKey];
    if (typeof src === "string") {
      ...
      if (targetWidth && targetHeight) {
        ctx.drawImage(imgTag, 0, 0, targetWidth, targetHeight);
      } else {
  ...
  playAudio(frame) {
    if (this._owner._forwardAnimating && this._owner._videoItem.audios instanceof Array) {
      this._owner._videoItem.audios.forEach((audio) => {
        if (
          audio.startFrame === frame &&
          this._bitmapCache[audio.audioKey] !== undefined &&
          typeof this._bitmapCache[audio.audioKey].play === "function"
        ) {
          const item = {
  ...
  resetShapeStyles(ctx, obj) {
    ...
    if (styles && styles.stroke) {
      ctx.strokeStyle = `rgba(${parseInt(styles.stroke[0] * 255)}, ${parseInt(styles.stroke[1] * 255)}, ${parseInt(
        styles.stroke[2] * 255
      )}, ${styles.stroke[3]})`;
    } else {
    ...
    if (styles && styles.fill) {
      ctx.fillStyle = `rgba(${parseInt(styles.fill[0] * 255)}, ${parseInt(styles.fill[1] * 255)}, ${parseInt(
        styles.fill[2] * 255
      )}, ${styles.fill[3]})`;
    } else {
